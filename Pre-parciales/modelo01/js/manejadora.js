...
async function mostrarUsuarios(filtro = ""){
    ...
    try {
        ...
        if(filtro === "argentinos") {
            users = data.filter(user => user.pais === "Argentina")
        } else if (filtro === "homonimos") {
            users = data.filter(user => user.nombre === "Juan")
        } 
        ...
        if(users.length === 0 ) {
            tabla += "<p>No se encontraron coincidencias</p>"
        }
...
async function eliminarUsuario(userId) {
    if(!confirm(`Esta seguro que desea eliminar el usuario ${userId}`)) return;
...
function mostrarArgentinos() {
    mostrarUsuarios("argentinos")
}

function mostrarHomonimos() {
    mostrarUsuarios("homonimos")
}